    ...
    public partial class AddModifyPatient : Page
    {
        ...
        private async void btnSubmit_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                // решение багов с введением даты рождения
                if (dateOfBirth != null)
                {
                    if (dateOfBirth.Value > DateTime.Today || dateOfBirth.Value < DateTime.Today.AddYears(-100))
                    {
                        status = false;
                        messageBuilder.Append("Выбранная дата не корректна");
                    }
                }
